...
def webhook_receiver():
    ...
    try:
        ...
        if parsed_data:
            # Store in MongoDB
            result = get_webhook_collection().insert_one(parsed_data)
...
def get_webhooks():
    ...
    try:
        ...
        webhooks = list(get_webhook_collection().find()
                       .sort('timestamp', -1)
                       .skip(offset)
                       .limit(limit))
...
def get_stats():
    ...
    try:
        total_webhooks = get_webhook_collection().count_documents({})
        
        # Get today's webhooks
        today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        today_webhooks = get_webhook_collection().count_documents({
        ...
        action_counts = list(get_webhook_collection().aggregate(pipeline))
